...
class OrderByLabelTest(fixtures.TablesTest):
    ...
    def test_group_by_composed(self):
        ...
        stmt = (
            select([func.count(table.c.id_m), expr])
            .group_by(expr)
            .order_by(expr)
        )
...
class LimitOffsetTest(fixtures.TablesTest):
    ...
    def test_simple_limit(self):
        table = self.tables.some_table
        self._assert_result(
            select([table]).order_by(table.c.id_m).limit(2),
            [(1, 1, 2), (2, 2, 3)],
        )
    ...
    def test_simple_offset(self):
        table = self.tables.some_table
        self._assert_result(
            select([table]).order_by(table.c.id_m).offset(2),
            [(3, 3, 4), (4, 4, 5)],
        )
    ...
    def test_simple_limit_offset(self):
        table = self.tables.some_table
        self._assert_result(
            select([table]).order_by(table.c.id_m).limit(2).offset(1),
            [(2, 2, 3), (3, 3, 4)],
        )
    ...
    def test_limit_offset_nobinds(self):
        ...
        table = self.tables.some_table
        stmt = select([table]).order_by(table.c.id_m).limit(2).offset(1)
        sql = stmt.compile(
    ...
    def test_bound_limit(self):
        table = self.tables.some_table
        self._assert_result(
            select([table]).order_by(table.c.id_m).limit(bindparam("l")),
            [(1, 1, 2), (2, 2, 3)],
            params={"l": 2},
        )
    ...
    def test_bound_offset(self):
        table = self.tables.some_table
        self._assert_result(
            select([table]).order_by(table.c.id_m).offset(bindparam("o")),
            [(3, 3, 4), (4, 4, 5)],
            params={"o": 2},
        )
...
class CompoundSelectTest(fixtures.TablesTest):
    ...
    def test_plain_union(self):
        table = self.tables.some_table
        s1 = select([table]).where(table.c.id_m == 2)
        s2 = select([table]).where(table.c.id_m == 3)

        u1 = union(s1, s2)
        self._assert_result(u1.order_by(u1.c.id_m), [(2, 2, 3), (3, 3, 4)])
    ...
    def test_limit_offset_selectable_in_unions(self):
        ...
        s1 = (
            select([table])
            .where(table.c.id_m == 2)
            .limit(1)
            .order_by(table.c.id_m)
        )
        s2 = (
            select([table])
            .where(table.c.id_m == 3)
            .limit(1)
            .order_by(table.c.id_m)
        )
    ...
    def test_order_by_selectable_in_unions(self):
        table = self.tables.some_table
        s1 = select([table]).where(table.c.id_m == 2).order_by(table.c.id_m)
        s2 = select([table]).where(table.c.id_m == 3).order_by(table.c.id_m)
    ...
    def test_distinct_selectable_in_unions(self):
        table = self.tables.some_table
        s1 = select([table]).where(table.c.id_m == 2).distinct()
        s2 = select([table]).where(table.c.id_m == 3).distinct()
    ...
    def test_limit_offset_in_unions_from_alias(self):
        ...
        self._assert_result(
            u1.select().limit(2).order_by(u1.c.id_m), [(2, 2, 3), (3, 3, 4)]
        )
        ...
        with config.db.connect() as conn:
            rows = {
                ...
                for value, in conn.execute(
                    select([some_table.c.id_m]).where(expr)
                )
            }
    ...
    def test_select_all(self):
        with config.db.connect() as conn:
            res = conn.execute(
                ...
                .order_by(self.tables.square.c.id_m)
            ).fetchall()
