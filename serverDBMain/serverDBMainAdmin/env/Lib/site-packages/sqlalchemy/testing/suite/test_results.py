...
class RowFetchTest(fixtures.TablesTest):
    ...
    def test_via_string(self):
        row = config.db.execute(
            self.tables.plain_pk.select().order_by(self.tables.plain_pk.c.id_m)
        ).first()
    ...
    def test_via_col_object(self):
        ...
        eq_(row[self.tables.plain_pk.c.id_m], 1)
    ...
    def test_row_with_dupe_names(self):
        result = config.db.execute(
            ...
            ).order_by(self.tables.plain_pk.c.id_m)
        )
        row = result.first()
    ...
    def test_row_w_scalar_select(self):
        ...
        s2 = select([datetable.c.id_m, s.label("somelabel")])
...
class ServerSideCursorsTest(
    ...
    def test_roundtrip(self):
        ...
        eq_(
            test_table.select().order_by(test_table.c.id_m).execute().fetchall(),
            [(1, "data1"), (2, "data2")],
        )
        test_table.update().where(test_table.c.id_m == 2).values(
