...
class LastrowidTest(fixtures.TablesTest):
    ...
    def test_last_inserted_id(self):
        ...
        pk = config.db.scalar(select([self.tables.autoinc_pk.c.id_s]))
...
class InsertBehaviorTest(fixtures.TablesTest):
    ...
    def test_empty_insert(self):
        ...
        r = config.db.execute(
            self.tables.autoinc_pk.select().where(
                self.tables.autoinc_pk.c.id_s != None
            )
        )
    ...
    def test_insert_from_select(self):
        ...
        config.db.execute(
            table.insert(inline=True).from_select(
                ("id", "data"),
                select([table.c.id_s + 5, table.c.data]).where(
    ...
    def test_insert_from_select_with_defaults(self):
        ...
        eq_(
            config.db.execute(
                select([table]).order_by(table.c.data, table.c.id_s)
            ).fetchall(),
            [
                (1, "data1", 5, 4),
                (2, "data2", 5, 4),
...
class ReturningTest(fixtures.TablesTest):
    ...
    def test_explicit_returning_pk_autocommit(self):
        ...
        with engine.begin() as conn:
            r = conn.execute(
                table.insert().returning(table.c.id_s), data="some data"
            )
        pk = r.first()[0]
        fetched_pk = config.db.scalar(select([table.c.id_s]))
        eq_(fetched_pk, pk)
